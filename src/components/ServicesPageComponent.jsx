...
function ServicesPageComponent({title, reverse, img, id, services}) {
  ...
  return (
    <section id={id} className={`lg:h-[100vh] px-8 flex flex-col ${reverse ? "lg:flex-row-reverse":"lg:flex-row"} lg:gap-4 justify-center items-center`}>
      ...
      <div className='mt-10  lg:w-3/4 '>
        ...
        <ul className='space-y-4 text-left text-gray-400 lg:grid lg:grid-cols-3'>
          {
          services.map((s, index) => {
            return <li key={index+s.title} className="flex items-center space-x-3 rtl:space-x-reverse">
            <img className='h-[100px] w-[100px] rounded-md lg:h-[150px] lg:w-[150px]' src={s.src} width={maxsize} height={maxsize} alt={s.title} />
            <span className='text-gray-700 lg:text-xl'>{s.title}</span>
          </li>
          })
          }
        </ul>
